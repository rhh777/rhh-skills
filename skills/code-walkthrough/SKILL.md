...
3. **用户响应**(经 `AskUserQuestion` 收到后)→ 确认(暂记,正式落盘在 [沉淀])/ 追问(展开)/ 跳转(放下路线去别处,之后回来)。
...
## 提问风格(均通过 `AskUserQuestion` 呈现)
- 一次只问一个点,聚焦当前这一段。
- 选项措辞要好判断:用「符合预期 / 不符 / 想追问」这类干脆的判断式,别写成模糊的开放题。
- 用户选了"懂了 / 符合预期"就推进,别换着花样反复盘问。
